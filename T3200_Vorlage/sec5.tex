...
Der reale Einsatzkontext des entwickelten Systems ist durch eine industrieähnliche Umgebung charakterisiert, welche inhärent eine Vielzahl optischer Störfaktoren aufweist. Hierzu zählen insbesondere teilweise stark fluktuierende Lichtverhältnisse, ein unruhiger und nicht immer idealer Hintergrund sowie das Vorhandensein anderer zu detektierender Objekte im Erfassungsbereich des Sensors. Zusätzlich erschweren transiente Schattenwürfe und unvorhersehbare Reflexionen eine konstante visuelle Erfassung. Um das Erkennungssystem unter diesen anspruchsvollen Bedingungen so robust wie möglich zu gestalten, gleichzeitig jedoch den zeitlichen und inhaltlichen Rahmen dieser Arbeit nicht zu sprengen und die vorhandene Rechenleistung des Zielsystems nicht überzustrapazieren, fiel die technologische Entscheidung auf ein Haar-Cascade-Modell. Dieses spezifische Klassifikatorenmodell wurde explizit darauf trainiert, die menschliche Hand als Zielobjekt zu identifizieren. Durch die mathematische Implementierung verschiedener, kontrastbasierter Filter innerhalb der Berechnungs-Pipeline wird zudem gewährleistet, dass die Handgeometrie selbst beim Tragen von persönlichen Arbeitshandschuhen zuverlässig und resilient erkannt werden kann.
\begin{figure}
	\begin{tikzpicture}[node distance=1.8cm and 1.2cm, auto]
	
	\node [block] (gauss) {Gaußfilter zur\\Rauschreduzierung};
	\node [block, below=of gauss] (gray) {Graustufenfilter um\\Farbinformationen\\zu reduzieren};
	\node [block, below=of gray] (thresh) {Threshold um\\Schwarz-Weiß-Bild\\mit variablem\\threshold zu erstellen};
	
	% Verbindungen und Pfeile beschriften
	\draw [arrow] (gauss.north)++(-1.8,0) -- node[above, pos=0.2] {rohes Bild} (gauss.north);
	\draw [arrow] (gauss.south) -- (gray.north);
	\draw [arrow] (gray.south) -- (thresh.north);
	\draw [arrow] (thresh.south) -- node[above, pos=0.8] {Cascade-Algorithmus} ++(2.5,0);
	
\end{tikzpicture}
\caption{Flussdiagramm der Bildvorverarbeitungs-Pipeline für das ML-Modell.}
\label{fig:filterstruktur_pipeline}
\end{figure}
