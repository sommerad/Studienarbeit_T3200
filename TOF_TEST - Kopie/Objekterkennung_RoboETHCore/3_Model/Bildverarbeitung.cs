...
namespace Objekterkennung._3_Model
{
    public class Bildverarbeitung
    {
        ...
        private List<Point[]> aGefundeneObjektKonturen = new List<Point[]>();
        ...
        public Mat ImageProcessing(Mat pCleanImage)
        {
            int area = 0;
            this.aMidpoints.Clear();
            this.aGefundeneObjektKonturen.Clear();
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (!isDuplicate && isObjekt)
                {
                    ...
                    this.aGefundeneObjektKonturen.Add(boxPoints); //Punkte für Handerkennung merken
                }
            }

            filterMat = aGrayFilter.Clone();
            contours.Dispose();
            CvInvoke.Imshow("3.Objekterkennung", aGrayFilter);
            return aGrayFilter;
        }
        ...
        private CascadeClassifier aHandCascade;
        public event Func<Task> OnHandDetectedAsync;
        public void LoadHandModel(string pXmlFilePath)
        {
            try
            {
                this.aHandCascade = new CascadeClassifier(pXmlFilePath);
                System.Diagnostics.Debug.WriteLine("Hand-Modell erfolgreich geladen");
            }
            catch(Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Fehler beim Laden der XML-Datei: {ex.Message}");
            }
        }

        public Mat DetectHands(Mat pCleanImage, out bool pHandDetected)
        {
            Mat resultMat = pCleanImage.Clone();
            pHandDetected = false; 

            if(this.aHandCascade == null)
            {
                return resultMat;
            }
            ...
            CvInvoke.GaussianBlur(pCleanImage, aBlurFilter, new Size(5, 5), 0);
            CvInvoke.CvtColor(aBlurFilter, grayFilter, ColorConversion.Bgr2Gray);
            CvInvoke.Threshold(grayFilter, threshold, 70, 255, ThresholdType.BinaryInv | ThresholdType.Otsu);

            foreach (Point[] objEckpunkte in this.aGefundeneObjektKonturen)
            {
                using (VectorOfPoint vp = new VectorOfPoint(objEckpunkte))
                {
                    CvInvoke.FillPoly(threshold, vp, new MCvScalar(0));
                }
            }
            int imgWidth = threshold.Width;
            int imgHeight = threshold.Height;
            Rectangle[] detectedHands = this.aHandCascade.DetectMultiScale(
           threshold,
           1.05,                  // scaleFactor: Sehr gründliche Suche (5% Schritte)
           6,                     // minNeighbors: Extrem tolerant (1 Treffer reicht)
           new Size(50, 50),      // minSize: Hand muss mindestens 50x50 Pixel groß sein
           new Size(imgWidth, imgHeight) // maxSize: Hand darf das GANZE Bild ausfüllen
           );

            if (detectedHands.Length > 0)
            {
                pHandDetected = true;
                Console.WriteLine("Hand erfolgreich erkannt.");
                if (OnHandDetectedAsync != null)
                {
                    Task.Run(async () => {
                        await OnHandDetectedAsync.Invoke();
                    });
                }
            }
            else Console.WriteLine("Keine Hand erkannt.");

            //-----------------------------------------------------------------\\
            // 3. Maske erstellen (nur der Hand-Bereich wird weiterverarbeitet)
            Mat mask = new Mat(threshold.Size, DepthType.Cv8U, 1);
            mask.SetTo(new MCvScalar(0)); // Maske komplett schwarz initialisieren

            foreach (Rectangle rect in detectedHands)
            {
                // Hand-Box in das farbige Ergebnisbild zeichnen
                CvInvoke.Rectangle(resultMat, rect, new MCvScalar(122, 122, 0), 2);
                // Hand-Box als weißen Bereich (255) in die Maske stanzen (-1 = ausgefüllt)
                CvInvoke.Rectangle(mask, rect, new MCvScalar(255), -1);
            }

            Mat img2 = new Mat();
            // Maske und Binärbild miteinander verschmelzen
            CvInvoke.BitwiseAnd(threshold, mask, img2);
            CvInvoke.GaussianBlur(img2, img2, new System.Drawing.Size(7, 7), 0);

            // 4. Konturen im isolierten Hand-Bereich finden
            using (VectorOfVectorOfPoint contours = new VectorOfVectorOfPoint())
            {
                CvInvoke.FindContours(img2, contours, null, RetrType.Tree, ChainApproxMethod.ChainApproxSimple);

                if (contours.Size > 0)
                {
                    // Um Rauschen zu vermeiden, suchen wir die größte Kontur im Bild (die echte Hand)
                    ...
                    VectorOfPoint cnt = contours[largestContourIdx];

                    // Konturen in das finale Bild zeichnen (Cyan)
                    CvInvoke.DrawContours(img2, contours, largestContourIdx, new MCvScalar(255, 255, 0), 3);
                }
            }
            //-----------------------------------------------------------------\\

            // Zeigt das reine Binärbild, in dem die KI nach der Hand sucht
            CvInvoke.Imshow("1. Was die KI sieht (Threshold)", threshold);

            // Zeigt die isolierte Hand, auf der die Konturen berechnet werden
            CvInvoke.Imshow("2. Extrahierte Hand (img2)", img2);
            return resultMat;
            //return grayFilter;
